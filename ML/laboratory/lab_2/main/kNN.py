...
class kNN():
    ...
    p : float, default=2
        Параметр мощности для метрики Минковского. Когда p = 1, это эквивалентно использованию manhattan_distance (l1) и euclidean_distance (l2) для p = 2. Ожидается, что параметр будет положительный.
    '''
    def __init__(self,
                k: int=5,
